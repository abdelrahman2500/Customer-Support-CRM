 ...
 * / injected `Logger` call site across the codebase is unchanged and
 * automatically routes through this.
 *
 * Every emitted line merges in the current request's correlation id (via
 * `CorrelationIdStore`, set for the request's lifetime by
 * `RequestIdMiddleware`) when one is active, and the Nest `context`
 * argument (e.g. a class name) as a bound `context` field. No
 * `pino-http`/automatic access-log middleware — this is application
 * logging only (see this story's own plan doc, "Non-goals").
 */
@Injectable()
