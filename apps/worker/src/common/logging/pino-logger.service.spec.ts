...
describe("PinoLoggerService", () => {
  ...
  it("includes the Nest context as a bound field", () => {
    service.log("hello", "MyProcessor");

    expect(pinoInstance.info).toHaveBeenCalledWith({ context: "MyProcessor" }, "hello");
  });

  it("merges the active correlation id (bound by a job's own processing) into the bindings", () => {
    CorrelationIdStore.run("job-1", () => service.log("hello", "MyProcessor"));

    expect(pinoInstance.info).toHaveBeenCalledWith(
      { context: "MyProcessor", correlationId: "job-1" },
      "hello",
    );
  });
  ...
  it("routes error() through pino's error level, including the trace field", () => {
    service.error("boom", "stack trace here", "MyProcessor");

    expect(pinoInstance.error).toHaveBeenCalledWith(
      { context: "MyProcessor", trace: "stack trace here" },
      "boom",
    );
  });
