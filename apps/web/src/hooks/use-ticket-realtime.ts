 ...
 * `RealtimeGateway`/`TicketRealtimeListener`, unmodified) and, on either of
 * the two events that room already relays, invalidates this ticket's
 * TanStack Query cache entries rather than trusting the socket payload as
 * the source of truth — the same "re-fetch by id" caution the backend's own
 * listeners already use. Does **not** join `branch:{id}:notifications`
 * (Story 22) or any `agent:{id}:presence` room — both explicitly out of
 * scope for this story. Connects on mount, disconnects on unmount/id change.
 */
