...
import { KbCategoriesView } from "./kb-categories-view";
import {
 ...
 * `ticket-categories-view.spec.tsx`'s exact convention: no `next-intl` mock
 * — every test renders through a real `NextIntlClientProvider` with the
 * actual `en.json`/`ar.json` messages.
 */
vi.mock("@/hooks/use-kb-categories", () => ({
  useManagedKbCategoriesQuery: vi.fn(),
  useCreateKbCategoryMutation: vi.fn(),
  useUpdateKbCategoryMutation: vi.fn(),
}));

const mockedUseManagedKbCategoriesQuery = vi.mocked(useManagedKbCategoriesQuery);
const mockedUseCreateKbCategoryMutation = vi.mocked(useCreateKbCategoryMutation);
const mockedUseUpdateKbCategoryMutation = vi.mocked(useUpdateKbCategoryMutation);
...
describe("KbCategoriesView", () => {
  ...
  it("shows a loading state while the categories query is pending", () => {
    mockedUseManagedKbCategoriesQuery.mockReturnValue(queryResult({ isLoading: true }) as never);

    renderView();

    expect(screen.getByText("Knowledge Base Categories")).toBeInTheDocument();
  ...
  it("shows an error state with a retry action", () => {
    ...
    expect(screen.getByText("Couldn't load Knowledge Base categories.")).toBeInTheDocument();
  ...
  it("shows the empty state when there are zero categories", () => {
    ...
    expect(screen.getByText("No Knowledge Base categories yet.")).toBeInTheDocument();
    expect(screen.queryByRole("table")).not.toBeInTheDocument();
  });
  ...
  it("commits a category rename on blur when the name changed", () => {
    ...
    mockedUseUpdateKbCategoryMutation.mockReturnValue(mutationResult({ mutate }) as never);
  ...
  describe("inline mutation errors", () => {
    it("renders a 403-specific message when the update mutation is rejected with 403", () => {
      ...
      mockedUseUpdateKbCategoryMutation.mockReturnValue(
  ...
  describe("create-category form", () => {
    ...
    it("shows the backend's own message inline on a rejected submission and preserves the entered value", async () => {
      ...
      expect(
        await screen.findByText("A Knowledge Base category with this name already exists"),
      ).toBeInTheDocument();
