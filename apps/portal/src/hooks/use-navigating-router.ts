...
/**
 * Story 171 — a drop-in replacement for `next/navigation`'s `useRouter()`
 ...
 *
 * Call-site opt-in, not a global patch: this app has no `url-filters.ts`
 * equivalent, so unlike `apps/web` there is no same-pathname filter-sync
 * `replace()` to exclude and every navigating call site imports this hook — same return shape as the real `useRouter()` (this only wraps
