...
Two hosts under one registrable domain are same-site even though they are
different _origins_ (CORS still applies to those — that is `CORS_ORIGINS`'s
job, not this setting's). Only a genuinely different registrable domain,
including the PaaS-supplied domains many platforms hand out per service,
makes the request cross-site and forces `none`.
...
| `S3_ENDPOINT` / `S3_ACCESS_KEY` / `S3_SECRET_KEY` | **yes in production**  | Attachments. Their schema defaults are the local MinIO container and MinIO's `minioadmin` dev credentials, which are never valid in production — so the API refuses to start in production unless all three are set explicitly. (`S3StorageService.onModuleInit` calls the endpoint at bootstrap, so before this check a missing value crash-looped the container on an `ECONNREFUSED` naming localhost.) |
| `S3_BUCKET`                                       | no (`crm-attachments`) | A bucket name is not environment-specific, and the storage service creates the bucket if it is absent.                                                                                                                                                                                                                                                                                                    |
