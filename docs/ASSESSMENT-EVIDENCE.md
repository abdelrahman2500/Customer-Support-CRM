...
| API keys wired to a production endpoint | The guard (HMAC-hashed, scoped, tenant-bound) is implemented and unit-tested (RM-22), but no real business endpoint currently accepts one — only an internal test fixture exercises it | `apps/api/src/common/auth/api-key.guard.ts`; `apps/api/src/test-fixtures/test-machine-route.controller.ts` |
| Knowledge Base retrieval/RAG (vector-based grounding) for AI ticket-assist and the portal chatbot | Explicitly out of scope for the current AI implementation, which is a human-in-the-loop assist layer, not an autonomous/grounded agent. A KB-grounding story (`ai-chat-kb-grounding`, Story 117) has a plan and intake document but no implementation commit | `.squad/plans/ai-chat-kb-grounding/117-story-ai-chat-kb-grounding.md` (all acceptance-criteria checkboxes unchecked; no corresponding commit in `git log`) |
